...
public class ReservaController {
    ...
    private void filtrarPorEstado() {
        ...
        List<Reserva> filtradas = reservas.stream()
            .filter(r ->
                ...
                (estado.equals("Canceladas") && false) // Cambiar si se añade estado cancelado
            )
    ...
    private void mostrarTotalPagar() {
        String id = view.getReservaSeleccionada();
        if (id == null) {
            view.mostrarMensaje("Seleccione una reserva para ver el total a pagar");
            return;
        }

        Reserva reserva = model.obtenerReserva(id);
        if (reserva == null) {
            view.mostrarMensaje("Reserva no encontrada");
            return;
        }
